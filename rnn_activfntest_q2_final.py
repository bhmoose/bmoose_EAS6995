...
class CharRNN(nn.Module):
    def __init__(self, vocab_size, hidden_size, dropout_rate, nonlinearity):
        super(CharRNN, self).__init__()
        self.hidden_size = hidden_size
        self.rnn = nn.RNN(vocab_size, hidden_size, dropout=dropout_rate, nonlinearity=nonlinearity, num_layers = 2, batch_first=True)
...
for nonlinearity in ['tanh', 'relu']:
    print('Starting nonlinearity = ' + nonlinearity)
    ...
    textfile = open('rnn_activ_text_' + nonlinearity + '.txt', 'w+')
    lossfile = open('rnn_activ_loss_' + nonlinearity + '.txt', 'w+')

    # Initialize model, loss function, optimizer
    model = CharRNN(vocab_size, hidden_size, dropout_rate, nonlinearity).to(device)  #Initialize model
