...
for dr in [0, 0.3, 0.8]:
    print("Starting dropout rate = " + str(dr))

    vocab_size = len(chars)
    hidden_size = 100
    seq_length = 25
    learning_rate = 0.01
    dropout_rate = dr #Set dropout rate

    num_epochs = 100
    batch_size = 64

    #Create files to store output
    textfile = open('rnn_dropout_text_' + str(dropout_rate) + '.txt', 'w+')
    lossfile = open('rnn_dropout_loss_' + str(dropout_rate) + '.txt', 'w+')
